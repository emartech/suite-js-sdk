...
describe('SuiteAPI Contact List endpoint', function() {
  ...
  describe('#fetch', function() {
    ...
    testApiMethod(ContactListApi, 'fetch').withArgs({
      contact_list_id: 2,
      next: '/contactlist/330/contactIds?$skiptoken=750&$top=1000'
    }).shouldGetResultFromEndpoint('v2/contactlist/330/contactIds?$skiptoken=750&$top=1000');

    testApiMethod(ContactListApi, 'fetch').withArgs({
      contact_list_id: 2,
      next: null
    }).shouldGetResultFromEndpoint('/contactlist/330/contactIds');

    testApiMethod(ContactListApi, 'fetch').withArgs({
      contact_list_id: 2,
      $skiptoken: null
    }).shouldGetResultFromEndpoint('/contactlist/330/contactIds?$skiptoken=750');
  });
